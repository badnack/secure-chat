...
public class Client {
    ...
    public static void MakeDir(String name, Path path){
        try{
            Directory.MakeDirectory(path.KEYDIRECTORY + name);
        }catch(IOException x ){
	...
	Path path = new Path();
        ...
        try{
            ...
            while(true){
                ...
                if(ch == 0) {
                    ...
                    if(!login.newUser(cred[0],cred[1],path)){
                ...
                if(ch == 1) {
                    ...
                    if(!login.LoadUser(cred[0],cred[1],path)){
		...
		cred=str.split(":");	
                ...
                cc = new ClientThread (user,false,path);
                cc.start();
                break;
